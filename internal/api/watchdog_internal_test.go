...
func TestWatchdogDecisionUsesDashboardPredicate(t *testing.T) {
	...
	cases := []struct {
		...
		// rpoStatus "ok", "warn", "never" and "off" are not reported.
		...
		// Overdue means older than two periods.
		...
		// A recorded episode is cleared as soon as the domain is current.
...
// watchdogTestService returns a Service over an in-memory store with a counting
// webhook as the only notify channel, one successful container backup and a
// daily containers schedule, plus the store, the webhook hit counter and the
// unix time of the seeded success.
...
func TestWatchdogNotifiesOncePerEpisode(t *testing.T) {
	...
	// Day 3: the seeded success is three periods old, so it is overdue.
	...
	// Day 4: same last success, same episode.
	...
	// Checked at a time when the backup is current, the episode is cleared.
	...
	// A new overdue episode notifies again.
	if err := svc.runWatchdogAt(ctx, seeded+3*day); err != nil {
		t.Fatalf("runWatchdogAt: %v", err)
	}
...
// TestWatchdogMutedPolicyIsSilent checks that muted notifications record no
// episode, so turning them on later still reports a long-running overdue state.
...
// TestWatchdogCoversEverythingOnlyDomains checks that a domain backed up only
// by the Backup Everything pass still gets an overdue alert.
func TestWatchdogCoversEverythingOnlyDomains(t *testing.T) {
	...
	settings.EverythingSchedule = "daily 03:00"
