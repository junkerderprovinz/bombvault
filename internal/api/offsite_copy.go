...
// keptObservation is what an import puts back for a target it writes back
// unchanged: when it was listed and aged, and what it held.
type keptObservation struct {
	obs    store.TargetObservation
	copies []store.ItemCopies
}

// observationsToKeep collects the observations of every target the import writes
// back with the same id, domain and location. Deleting the rows drops them, and
// the next pass would otherwise take each of those targets for never listed.
func (s *Service) observationsToKeep(current []store.OffsiteTarget, views []offsiteTargetView) ([]keptObservation, error) {
	byID := make(map[string]offsiteTargetView, len(views))
	for _, v := range views {
		byID[strings.TrimSpace(v.ID)] = v
	}
	var out []keptObservation
	for _, t := range current {
		v, ok := byID[t.ID]
		if !ok || v.Domain != t.Domain || importedLocation(t.Repo, strings.TrimSpace(v.Repo)) != t.Repo {
			continue
		}
		obs, listed, err := s.store.TargetObservationFor(t.Domain, t.ID)
		if err != nil {
			return nil, err
		}
		if !listed {
		...
		all, err := s.store.ItemCopiesForDomain(t.Domain)
		if err != nil {
			return nil, err
		}
		k := keptObservation{obs: obs}
		for _, c := range all {
			if c.TargetID == t.ID {
				k.copies = append(k.copies, c)
			}
		}
		out = append(out, k)
	}
