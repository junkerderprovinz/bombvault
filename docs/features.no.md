...
- **Destinasjoner:** en lokal sti, eller ekstern. SMB/CIFS og NFS (monter delingen på Unraid og pek en sikkerhetskopisti mot den), native restic-backender uten rclone (`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`), eller en hvilken som helst rclone-remote via `rclone:<remote>:<bucket>/path`. All legitimasjon lagres kryptert.
