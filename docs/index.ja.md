...
BombVault は、Docker コンテナと KVM/libvirt VM の**バックアップと完全な災害復旧**のための、セルフホスト型で Unraid ネイティブな Web アプリです。単一のマルチアーキテクチャ Docker コンテナとして動作し、モダンなダークテーマの Web UI を備え、バックアップ、スケジュール設定、検証、復元というライフサイクル全体を扱います。
