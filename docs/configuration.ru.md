...
- **Бэкенды:** SMB/CIFS и NFS (смонтируйте общий ресурс и укажите на него Путь резервных копий), нативные бэкенды restic без rclone (`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`) или любое удалённое хранилище rclone (`rclone:<remote>:<bucket>/path`).
