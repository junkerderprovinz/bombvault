...
- **대상:** 로컬 경로 또는 오프사이트. SMB/CIFS와 NFS(Unraid에 공유를 마운트하고 백업 경로를 그곳으로 지정), rclone 없는 네이티브 restic 백엔드(`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`), 또는 `rclone:<remote>:<bucket>/path`를 통한 모든 rclone 원격. 모든 자격 증명은 암호화되어 저장됩니다.
