...
- **Bestemmingen:** een lokaal pad, of off-site. SMB/CIFS en NFS (mount de share op Unraid en wijs er een Backup Path naar), native restic-backends zonder rclone (`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`), of elke rclone-remote via `rclone:<remote>:<bucket>/path`. Alle inloggegevens worden versleuteld opgeslagen.
...
- **Plaatsing per item.** Elke kaart van een container, VM en bestandsset kiest **Lokaal**, **Lokaal + off-site** of **Alleen off-site**, en onder Lokaal + off-site welke doelen een kopie krijgen. Een share die al op een NAS staat, hoeft niet ook nog naar B2. De locatie ligt vast vanaf de eerste back-up, de kopieën kunnen op elk moment veranderen, en de kaart zegt hoeveel locaties het item bevatten en of aan 3-2-1 wordt voldaan. Zie [Plaatsing per item](offsite-recovery.md#placement).
