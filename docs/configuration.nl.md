...
- **Optionele wachtwoordbeveiliging** (Instellingen, Beveiliging): stel een wachtwoord in om inloggen te vereisen, wis het om het uit te schakelen. Standaard uit voor gebruik in een vertrouwd LAN. Het wachtwoord wordt opgeslagen met Argon2id over een met `APP_KEY` gepeperde waarde, dus een gekopieerde `/config` is zonder de sleutel waardeloos en mét sleutel traag aan te vallen. Een nieuw wachtwoord heeft minstens 12 tekens nodig; een bestaand korter wachtwoord blijft werken tot het wordt gewijzigd. Sessies zijn ondertekend (HMAC afgeleid van `APP_KEY`) en een wachtwoordwijziging maakt ze ongeldig; inloggen is beperkt tot vijf mislukte pogingen per minuut per client.
- **Tweefactorauthenticatie** (Instellingen): een tijdcode uit een authenticator-app boven op het wachtwoord, plus acht eenmalige herstelcodes die bij het inschakelen één keer worden getoond. Het gedeelde geheim wordt versleuteld met `APP_KEY` bewaard, en uitschakelen vereist een actuele code.
