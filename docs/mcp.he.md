...
3. השאר את **אפשר התחלת גיבויים** מופעל, או כבה אותו למפתח שצריך רק לקרוא. אפשר לשנות זאת אחר כך באריח של המפתח, והשינוי חל מהבקשה הבאה של העוזר, בלי חיבור מחדש.
...
**יומן** באריח פותח את מה שהמפתח עשה. קודם מופיעים הגיבויים שהתחיל, כל אחד עם המצב שלו וקישור להרצה ביומן הפעילות בלוח הבקרה. מתחתם הקריאות שלו, החדשות קודם, עם הכלי ומה עלה בגורל הקריאה. דחייה מציינת את הסיבה: המפתח רשאי רק לקרוא, שומר השמירה עצר את הגיבוי, גיבוי אחר כבר רץ, הפריט גובה דרך MCP לפני כמה דקות, או שהמפתח שלח יותר מדי בקשות. ביטול מקשר להרצה שבה עסק.

BombVault שומר את 200 הרשומות החדשות ביותר של כל מפתח עד 30 יום. לכל קריאה הוא שומר את הכלי, התוצאה וההרצה שביטול ציין. הוא אף פעם לא שומר את מה שהעוזר שלח, וגם לא את המפתח או טביעת האצבע שלו. חבילת האבחון רק סופרת את הרשומות, וייצוא ההגדרות משמיט אותן.
