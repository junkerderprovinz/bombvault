...
- **Бекенди:** SMB/CIFS та NFS (змонтуйте спільний ресурс і вкажіть на нього Шлях резервних копій), нативні бекенди restic без rclone (`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`) або будь-яке віддалене сховище rclone (`rclone:<remote>:<bucket>/path`).
