...
!!! note "Επαναφορά από οποιοδήποτε σημείο"
...
## Τοποθέτηση ανά στοιχείο {#placement}

Κάθε κάρτα container, VM και συνόλου φακέλων έχει μια γραμμή **Τοποθέτηση** με τρία τμήματα:

- **Τοπικό** γράφει το στοιχείο στο αποθετήριο που εμφανίζεται κάτω από το **Αποθηκευμένο σε** και δεν το αντιγράφει πουθενά. Χρησιμοποιήστε το για δεδομένα που έχουν ήδη ένα δεύτερο αντίγραφο, για παράδειγμα έναν κοινόχρηστο πόρο που βρίσκεται σε NAS.
- **Τοπικό + εκτός τόπου** το γράφει και εκεί και το αντιγράφει στους στόχους που είναι επιλεγμένοι κάτω από το **Αντιγραφή σε**, ένα chip ανά στόχο εκτός τόπου του τομέα. Αποεπιλέξτε ένα chip, και εκείνος ο στόχος δεν παίρνει πια τίποτα καινούργιο από αυτό το στοιχείο.
- **Μόνο εκτός τόπου** γράφει το στοιχείο απευθείας στο σημείο κάτω από το **Αποστολή σε**: ένα άμεσο αποθετήριο δίπλα σε έναν στόχο εκτός τόπου, ή ένα απομακρυσμένο αποθετήριο που ρυθμίζετε στις Ρυθμίσεις, Διαδρομές και αποθήκευση, Αποθετήρια.
...
Η επιλογή του άμεσου αποθετηρίου ενός στόχου κάτω από το Μόνο εκτός τόπου ανοίγει ένα παράθυρο διαλόγου με μια προτεινόμενη τοποθεσία δίπλα στον στόχο, για παράδειγμα `b2:bucket:containers-direct`, και μια δοκιμή σύνδεσης που δεν δημιουργεί τίποτα. Το **Δημιουργία και χρήση** δημιουργεί το αποθετήριο και κατευθύνει το στοιχείο σε αυτό. Ένα άμεσο αποθετήριο παίρνει το κλειδί, την κλάση αποθήκευσης, τα όρια, τη ρύθμιση append-only και τη διατήρηση του στόχου, και αλλάζει μαζί τους· η κάρτα Αποθετήρια το εμφανίζει ως μόνο για ανάγνωση. Όταν ένα νέο κλειδί για τον στόχο δεν μπορεί να το ανοίξει, το άμεσο αποθετήριο κρατά το κλειδί που έχει, και η αποθήκευση το αναφέρει. Τα στιγμιότυπά του φέρουν την ετικέτα `bv:direct`, και κάθε άλλη εκτέλεση διατήρησης τα αφήνει, ώστε ένα άμεσο αποθετήριο που έχασε τη σύνδεσή του με τον στόχο του δεν γερνά ποτέ σύμφωνα με τους τοπικούς κανόνες. Ένα κλειδί B2 που περιορίζεται στον δικό του φάκελο του στόχου δεν μπορεί να φτάσει στον διπλανό φάκελο· περιορίστε αντ' αυτού το κλειδί στον φάκελο πάνω από τον στόχο.
...
!!! note "Μια επιτυχής διαγραφή στο `/locks/` είναι αναμενόμενη"
    Το append-only δεν σημαίνει ότι δεν μπορεί πλέον να διαγραφεί τίποτα. Το restic πρέπει να παίρνει και να απελευθερώνει τα δικά του κλειδώματα, γι' αυτό το `/locks/` παραμένει σκόπιμα εγγράψιμο και διαγράψιμο. Τα στιγμιότυπα και τα δεδομένα πίσω τους, δηλαδή ακριβώς ο στόχος ενός ransomware, δεν μπορούν να αφαιρεθούν. Αν ελέγξεις μόνος σου την απομακρυσμένη πλευρά, μια διαγραφή που πετυχαίνει στο `/locks/` είναι σωστή συμπεριφορά και όχι κενό ασφαλείας.
...
**1. Στο VAULT, στήσε τον διακομιστή append-only.** Στο BombVault στο TOWER πήγαινε *Ρυθμίσεις → Εκτός έδρας → καθοδηγούμενη ρύθμιση*, διάλεξε **rest-server** και δημιούργησε τη συνταγή. Αντίγραψε την καρτέλα **Πρότυπο Unraid (XML)**, αποθήκευσέ την στο VAULT ως `/boot/config/plugins/dockerMan/templates-user/my-rest-server.xml`, μετά *Docker → Add Container* και διάλεξε **rest-server** από τη λίστα προτύπων. Πριν το ξεκινήσεις, γράψε τη γραμμή `htpasswd` που εμφανίζεται στο `/mnt/user/appdata/rest-server/.htpasswd` στο VAULT. Ο κωδικός μιας χρήσης εμφανίζεται μία φορά και δεν αποθηκεύεται ποτέ, αντίγραψέ τον τώρα. Αυτή η γραμμή φέρει το ίδιο συνθηματικό, ήδη κατακερματισμένο με bcrypt για εσένα: το απλό κείμενο πηγαίνει στα διαπιστευτήρια REST στο TOWER, η κατακερματισμένη γραμμή στο `.htpasswd` στο VAULT. Δεν χρειάζεται να κατακερματίσεις τίποτα.
...
!!! note "Τα αντίγραφα εκτός τόπου περιμένουν μετά από ανακατασκευή"
    Όταν το βήμα 4 ανακατασκευάζει στοιχεία χωρίς τις παλιές ρυθμίσεις, η αναπαραγωγή εκτός τόπου αυτών των τομέων μπαίνει σε παύση μέχρι να επιβεβαιωθεί η προεπιλογή τοποθέτησης. Δείτε [Τοποθέτηση ανά στοιχείο](#placement).
