...
| `DB_DUMP_MAX_HOURS` | Nu | Orele în care un dump automat de bază de date poate rula înainte să fie oprit. Gol (implicit) înseamnă `6`; sunt permise valori de la `1` la `48`, iar limita rămâne mereu cu cel puțin o oră sub `BACKUP_MAX_HOURS`, astfel încât un dump lung să fie tăiat de propria limită și raportat ca atare, în loc să tragă backupul după el. Un dump care nu mai avansează este oprit mai devreme, după `BACKUP_STALL_HOURS`. Un dump oprit eșuează de unul singur, iar backupul containerului continuă. Pe Unraid adaugi variabila la containerul BombVault cu **Add another Path, Port, Variable**. |
