...
พาธของรีพอสิทอรีการสำรองข้อมูลตั้งค่าเริ่มต้นเป็น `/mnt/user/bombvault/{container,vms,flash,config,files,zfs}` สร้างขึ้นในการสำรองข้อมูลครั้งแรก เปลี่ยนตำแหน่งได้ทุกเมื่อใน **Settings, Backup paths**
...
- Reverse proxy ที่อยู่หน้า BombVault ต้องส่งต่อส่วนหัว `Authorization` หรือ `X-API-Key` ไปยัง `/mcp` และต้องไม่บัฟเฟอร์คำตอบ มิฉะนั้นผู้ช่วยจะเชื่อมต่อไม่ได้ ดู [เซิร์ฟเวอร์ MCP](mcp.md#tls)
...
เซิร์ฟเวอร์ MCP ไม่ต้องใช้ตัวแปรสภาพแวดล้อมใด คุณเปิดใช้งานได้ด้วยการสร้างคีย์ที่ **การตั้งค่า, ระบบ, เซิร์ฟเวอร์ MCP** และเซิร์ฟเวอร์จะตอบที่ `/mcp` บนพอร์ตเดียวกับเว็บอินเทอร์เฟซ (เช่น `https://192.168.1.10:3443/mcp`) เมื่อไม่มีคีย์ที่ใช้งานอยู่ พาธนี้จะตอบ `404` ไคลเอนต์ ใบรับรอง และขีดจำกัดอธิบายไว้ที่ [เซิร์ฟเวอร์ MCP](mcp.md)
...
1. **Settings, System, Host SSH:** คัดลอกคีย์สาธารณะที่แสดง
...
- **ปลายทาง SSH ไม่ต้องติดตั้งอะไรที่ฝั่งไกล** `sftp:` ต้องการเพียงเซิร์ฟเวอร์ SSH เพิ่มคีย์สาธารณะจาก **Settings, System, Host SSH** (อยู่ที่ `/config/ssh/id_ed25519.pub` ด้วย) ลงใน `~/.ssh/authorized_keys` ของผู้ใช้ปลายทาง
...
แต่ละรายการมีความไวและระดับการแจ้งเตือนขั้นต่ำของตัวเองได้ ตั้งค่าได้ที่แท็บ **รายการ** ของหน้า **ความผิดปกติ** หรือในแผงของรายการนั้นเอง: ส่วนโฟลเดอร์ของคอนเทนเนอร์และการตั้งค่าของ VM (ทั้งสองอยู่ในโหมดขั้นสูง) ตัวแก้ไขโฟลเดอร์ของชุดโฟลเดอร์ และหน้า **Flash** กับ **สำรองข้อมูลตัวเอง** สำหรับรายการ ZFS จะอยู่ในตัวแก้ไขของรายการบนหน้า **ZFS** และใช้กับทุกชุดข้อมูลในทรีของรายการนั้น
