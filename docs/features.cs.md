...
- **Cíle:** místní cesta nebo mimo lokalitu. SMB/CIFS a NFS (připojte sdílenou složku v Unraidu a nasměrujte na ni Zálohovací cestu), nativní restic backendy bez rclone (`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`) nebo libovolný rclone remote přes `rclone:<remote>:<bucket>/path`. Všechny přihlašovací údaje se ukládají šifrovaně.
...
- **Umístění pro jednotlivé položky.** Každá karta kontejneru, VM a sady složek volí **Místní**, **Místní + mimo lokalitu** nebo **Jen mimo lokalitu**, a pod Místní + mimo lokalitu, které cíle dostanou kopii. Sdílená složka, která už leží na NAS, už nemusí jít i do B2. Umístění je pevné od první zálohy, kopie se mohou kdykoli změnit a karta ukazuje, na kolika lokalitách položka leží a jestli je splněno 3-2-1. Viz [Umístění pro jednotlivé položky](offsite-recovery.md#placement).
