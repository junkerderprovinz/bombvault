...
2. 在**设置**中，启用您想要的备份域（容器、虚拟机、闪存、配置、文件、ZFS 数据集）并选择一个强调色。
3. 在**容器**标签页，选择一个容器并点击**备份**以创建您的第一个还原点。仓库路径默认为 `/mnt/user/bombvault/{container,vms,flash,config,files,zfs}`，并在首次备份时创建。
