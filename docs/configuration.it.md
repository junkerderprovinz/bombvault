...
| `DB_DUMP_MAX_HOURS` | No | Ore per cui un dump automatico di database può girare prima di essere fermato. Vuoto (il valore predefinito) usa `6`; sono ammessi valori da `1` a `48`, e il limite resta sempre almeno un'ora sotto `BACKUP_MAX_HOURS`, così un dump lungo viene tagliato dal proprio limite e segnalato come tale invece di trascinarsi dietro il backup. Un dump che non avanza più viene fermato prima, dopo `BACKUP_STALL_HOURS`. Un dump fermato fallisce per conto suo e il backup del container prosegue. Su Unraid aggiungi la variabile al container BombVault con **Add another Path, Port, Variable**. |
