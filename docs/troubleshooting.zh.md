...
是一个未关闭的严重异常在保留它们：对象的源几乎为空、大幅缩小，或某次备份重新存储了大部分数据。从对象上的徽标打开该异常。如果数据缺失或已被加密，请先从链接的最后一个正常备份恢复。然后确认该异常，如果更改是您自己做的，则将其标记为预期之内，下一次运行会照常清理。保留预览会把这样的对象标记为已保留。对于 ZFS 对象，只有异常所指的数据集保留旧备份，树中的其他数据集照常清理。
