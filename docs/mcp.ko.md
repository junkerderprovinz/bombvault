...
BombVault는 키마다 기록을 최대 30일 동안 보관합니다. 시작, 취소, 거부, 오류는 최신 500건, 이와 별도로 성공한 읽기는 최신 200건입니다. 실행 중인 백업을 계속 조회하는 어시스턴트가 있어도 그 백업의 시작이 기록에서 밀려나지 않습니다. 호출마다 도구, 결과, 취소가 가리킨 실행만 저장합니다. 어시스턴트가 보낸 내용과 키, 키의 지문은 절대 저장하지 않습니다. 진단 번들에는 건수만 들어가고, 설정 내보내기에는 빠집니다.
