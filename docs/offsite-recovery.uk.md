...
!!! note "Відновлення з будь-якого місця"
...
## Розміщення для кожного елемента {#placement}

Кожна картка контейнера, VM і набору файлів має рядок **Розміщення** із трьома сегментами:

- **Локально** записує елемент у репозиторій, показаний під **Збережено на**, і нікуди його не копіює. Використовуй для даних, які вже мають другу копію, наприклад для спільного ресурсу, що лежить на NAS.
- **Локально + зовнішнє** записує його туди ж і копіює до цілей, позначених під **Копіювати в**, одна фішка на кожну зовнішню ціль домену. Зніми позначку з фішки, і ця ціль більше не отримає нічого нового від цього елемента.
- **Лише зовнішнє** записує елемент прямо в місце під **Надіслати в**: прямий репозиторій біля зовнішньої цілі або віддалений репозиторій, який ти налаштував у Налаштування, Шляхи та сховище, Репозиторії.
...
Вибір прямого репозиторію цілі під Лише зовнішнє відкриває діалог із запропонованим розташуванням поруч із ціллю, наприклад `b2:bucket:containers-direct`, і перевіркою з'єднання, яка нічого не створює. **Створити і використати** створює репозиторій і спрямовує елемент до нього. Прямий репозиторій переймає ключ, клас сховища, ліміти, налаштування append-only та зберігання цілі й змінюється разом із ними; картка Репозиторії показує його лише для читання. Коли новий ключ цілі не може його відкрити, прямий репозиторій зберігає той ключ, який має, і збереження про це повідомляє. Його знімки несуть мітку `bv:direct`, і кожне інше очищення зберігає їх, тож прямий репозиторій, що втратив зв'язок зі своєю ціллю, ніколи не старіє за локальними правилами. Ключ B2, обмежений власною текою цілі, не може дістатися до теки поруч із нею; натомість обмеж ключ текою над ціллю.
...
!!! note "Успішне видалення в `/locks/` очікуване"
    Append-only не означає, що більше нічого не можна видалити. restic має створювати та знімати власні блокування, тому `/locks/` навмисно лишається доступним для запису й видалення. Знімки та дані за ними, тобто саме те, на що націлений шифрувальник, видалити неможливо. Якщо ти сам перевіриш віддалену сторону, успішне видалення в `/locks/` є правильною поведінкою, а не дірою в захисті.
...
![Приймальна сторона, спостережувана лише на читання, з перевіркою цілісності на цій машині.](assets/screenshots/receiver.png)
...
!!! note "Зовнішні копії чекають після перебудови"
    Коли крок 4 перебудовує записи без старих налаштувань, зовнішня реплікація цих доменів призупиняється, доки не підтверджено значення за замовчуванням розміщення. Див. [Розміщення для кожного елемента](#placement).
...
!!! warning "Для отриманого репозиторію беріть ключ ВІДПРАВНОГО примірника"
