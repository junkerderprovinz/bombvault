...
לכל כרטיס container, VM וקבוצת קבצים יש שורת **מיקום** עם שלושה מקטעים:

- **מקומי** כותב את הפריט למאגר המוצג תחת **מאוחסן ב** ולא מעתיק אותו לשום מקום. השתמש בו לנתונים שכבר יש להם עותק שני, למשל שיתוף שיושב על NAS.
- **מקומי + מחוץ לאתר** כותב אותו גם שם ומעתיק אותו ליעדים המסומנים תחת **העתקה אל**, chip אחד לכל יעד מחוץ לאתר של הדומיין. בטל סימון של chip והיעד הזה לא מקבל דבר חדש מהפריט הזה.
- **מחוץ לאתר בלבד** כותב את הפריט ישירות למקום תחת **שליחה אל**: מאגר ישיר לצד יעד מחוץ לאתר, או מאגר מרוחק שהגדרת תחת הגדרות, נתיבים ואחסון, מאגרים.
...
בחירת המאגר הישיר של יעד תחת מחוץ לאתר בלבד פותחת תיבת דו-שיח עם מיקום מוצע לצד היעד, למשל `b2:bucket:containers-direct`, ובדיקת חיבור שלא יוצרת דבר. **צור והשתמש** יוצר את המאגר ומצביע את הפריט אליו. מאגר ישיר לוקח את המפתח, מחלקת האחסון, המגבלות, הגדרת ה-append-only והשמירה של היעד, ומשתנה איתם; כרטיס המאגרים מציג אותו לקריאה בלבד. כשמפתח חדש של היעד לא יכול לפתוח אותו, המאגר הישיר שומר את המפתח שיש לו וההצלה אומרת זאת. תמונות המצב שלו נושאות את התג `bv:direct`, וכל מעבר שמירה אחר משאיר אותן, כך שמאגר ישיר שאיבד את הקשר שלו ליעד לעולם לא מזדקן לפי הכללים המקומיים. מפתח B2 המוגבל לתיקייה של היעד עצמו לא יכול להגיע לתיקייה הסמוכה לה; הגבל את המפתח במקום זאת לתיקייה שמעל היעד.
...
![הצד המקבל, מנוטר לקריאה בלבד, עם בדיקת שלמות שרצה על המכונה הזו.](assets/screenshots/receiver.png)
