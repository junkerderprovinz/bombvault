...
נתיב הגיבוי של תחום (הגדרות, נתיבים ואחסון) אינו מוגבל לתיקייה מקומית: כוון אותו ישירות אל מאגר restic מרוחק (`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`, `rclone:remote:bucket/path`) ו-BombVault יגבה ישר לשם, בלי עותק מקומי נפרד ובלי שלב שכפול. זו צורה שונה באמת מהשכפול מחוץ לאתר שלמעלה: שם המאגר המקומי הוא הראשי והמאגר שמחוץ לאתר הוא ארכיון שלו כמיטב היכולת; כאן המאגר המרוחק **הוא** הראשי, והוא העותק היחיד כל עוד לא הגדרת לאותו תחום גם שכפול מחוץ לאתר (או מאגר מרוחק שני).
