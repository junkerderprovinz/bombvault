...
- **Mål:** en lokal sökväg, eller off-site. SMB/CIFS och NFS (montera resursen på Unraid och peka en säkerhetskopiesökväg mot den), native restic-backender utan rclone (`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`), eller valfri rclone-fjärr via `rclone:<remote>:<bucket>/path`. Alla uppgifter lagras krypterade.
...
- **Placering per objekt.** Varje kort för container, VM och filuppsättning väljer **Lokal**, **Lokal + utanför platsen** eller **Endast utanför platsen**, och under Lokal + utanför platsen vilka mål som får en kopia. En resurs som redan ligger på en NAS behöver inte längre gå till B2 också. Platsen är fast från den första säkerhetskopian, kopiorna kan ändras när som helst, och kortet säger hur många platser som har objektet och om 3-2-1 är uppfyllt. Se [Placering per objekt](offsite-recovery.md#placement).
