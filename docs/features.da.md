...
- **Destinationer:** en lokal sti, eller off-site. SMB/CIFS og NFS (montér share'en på Unraid, og peg en Backup Path mod den), native restic-backends uden rclone (`s3:...`, `rest:http://host:8000/repo`, `sftp:user@host:/repo`) eller en hvilken som helst rclone-remote via `rclone:<remote>:<bucket>/path`. Alle legitimationsoplysninger gemmes krypteret.
...
- **Placering pr. element.** Hvert container-, VM- og mappesæt-kort vælger **Lokal**, **Lokal + ekstern** eller **Kun ekstern**, og under Lokal + ekstern, hvilke destinationer der får en kopi. En deling, der allerede ligger på et NAS, skal ikke også til B2. Placeringen er fast fra første sikkerhedskopi, kopierne kan ændres når som helst, og kortet viser, hvor mange lokationer elementet holdes på, og om 3-2-1 er opfyldt. Se [Placering pr. element](offsite-recovery.md#placement).
