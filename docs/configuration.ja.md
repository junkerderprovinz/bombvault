...
| `TZ` | いいえ | スケジューラーのタイムゾーン（たとえば `Europe/Berlin`）。 **設定しない場合、すべてのスケジュールは UTC で実行されます**。02:30 に設定したスケジュールは現地時間ではなく 02:30 UTC に開始します。 Unraid では自分で設定する必要はありません。システムが自身のタイムゾーンをすべてのコンテナーに渡します。 |
