...
| `DB_DUMP_MAX_HOURS` | Nej | Timmar som en automatisk databasdump får köra innan den stoppas. Tomt (standard) använder `6`; tillåtna värden är `1` till `48`, och gränsen hålls alltid minst en timme under `BACKUP_MAX_HOURS`, så att en lång dump kapas av sin egen gräns och rapporteras som det i stället för att dra med sig säkerhetskopian. En dump som slutar göra framsteg stoppas tidigare, efter `BACKUP_STALL_HOURS`. En stoppad dump misslyckas för sig själv och containerns säkerhetskopiering fortsätter. På Unraid lägger du till variabeln på BombVault-containern med **Add another Path, Port, Variable**. |
