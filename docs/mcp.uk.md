...
| `list_items` | Кожен захищений контейнер, VM і набір тек, флешка і налаштування застосунку, з розкладом, тим, що зупиняє копіювання, останньою копією та її тривалістю; контейнери баз даних показують і останній дамп | читання |
| `list_runs` | Історія запусків, нові згори, з фільтром за доменом, елементом, статусом, видом і часом | читання |
| `list_restore_points` | Точки відновлення одного елемента з його основного репозиторію, а для контейнера ще й дампи його баз даних | читання |
