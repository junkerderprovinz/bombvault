...
未解決の重大な異常がそれを止めています。項目のソースがほぼ空になった、大きく縮んだ、またはバックアップがデータの大部分を保存し直したという異常です。項目のバッジから異常を開いてください。データが欠けている、または暗号化されている場合は、まずリンクされた最後の正常なバックアップから復元します。その後で異常を確認済みにするか、変更が自分によるものなら想定どおりと印を付けると、次の実行から通常どおり整理されます。保持のプレビューでは、このような項目は保持と表示されます。
