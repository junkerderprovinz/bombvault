...
| `TZ` | No | Timezone for the scheduler (e.g. `Europe/Berlin`). **Leave it unset and every schedule runs in UTC**: one set to 02:30 then fires at 02:30 UTC rather than on your own wall clock. The boot log states which way it resolved. Note the trade-off either way: a zone with daylight saving skips one run in spring and runs twice in autumn, while UTC does neither but drifts an hour against your local clock twice a year. |
