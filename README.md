...
- **Answers your AI assistant.** The built-in MCP server lets Claude Code, Claude Desktop or another MCP client read backup status, coverage, run history, restore points (database dumps and ZFS datasets included) and the open anomalies, and, with a key that allows it, start a backup. Every client gets a key of its own; restores, deletions and settings stay in the web interface. See [docs/mcp.md](docs/mcp.md).
...
| **ZFS datasets** | A dataset together with every dataset below it, from **one ZFS snapshot**, so all of them come from the same instant. Each dataset is stored with restic like a folder: deduplicated, browsable, single files restorable. **New child datasets are picked up on their own** and named on the next run, single children can be left out, and a child that cannot be read is skipped and named, never silently. Optionally stop containers or run a command for just the instant of the snapshot; the backup then reads the snapshot while the apps run again. An in-place restore takes a **safety snapshot** first, kept until you delete it. Needs the same SSH link as VM backups. Volumes are not included: a VM's volume is backed up with its VM, a volume without a VM is not backed up yet |
...
- **Backup-health heatmap** — a GitHub-contributions-style calendar of per-day backup outcomes per domain (green = all OK, red = a failure), with a Containers / VMs / Flash / Config / Files / ZFS toggle.
...
- **Anomalies**: every backup of a container, VM, folder set, ZFS dataset, database dump, the flash drive and the self-backup is compared with its own history, and what does not fit shows up on a dashboard card, an **Anomalies** page and a badge on the item. Most checks start after 10 backups; an almost empty source, a backup that stored most of the data again and failures count from the first one. Critical findings about lost data pause the deletion of that item's old backups until you acknowledge them or mark them as expected, and point at the last good backup to restore. Sensitivity and which findings notify are set globally on **Settings → Integrity**, and per item on the **Anomalies** page or in the item's own panel. See [Features](docs/features.md) for what each check looks at.
...
> **Settings → System → Host SSH** → copy the shown public key → append it to
