/**
 * The two donation buttons the README needs beside the Buy Me a Coffee one.
 *
 ...
 * with a 38.2 corner, and the README renders every button at width="220". A
 ...
 * design grid that button-docs.svg already uses (scaled by 1.830217 and offset
 * to sit centred in the larger box).
 *
 ...
 *
 * Run: node .github/assets/gen-donate-buttons.mjs
 */
...
<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}">
  ...
  <g transform="translate(${DX},${DY}) scale(${SCALE})">
    <g transform="translate(${startX},34) scale(${markScale})" fill="${ink}">
      <path d="${mark}"/>
    </g>
...
/** The word's real painted width, by rendering it alone and trimming. */
function textWidth(label) {
  ...
  const img = new Resvg(probe, { fitTo: { mode: "width", value: 900 } }).render();
  const png = img.asPng();
  const tmp = join(__dir, ".probe.png");
  writeFileSync(tmp, png);
  const box = execSync(`magick "${tmp}" -trim -format "%w" info:`).toString().trim();
  unlinkSync(tmp);
  return Number(box);
}
...
for (const b of BUTTONS) {
  ...
  const startX = Math.round((460 - (MARK + GAP + tw)) / 2);
