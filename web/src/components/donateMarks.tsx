// ---------------------------------------------------------------------------
// The marks the donation card and the crypto window wear ([3554]).
//
// BRAND MARKS, and that is why they live here rather than in glyphs.tsx: the
// house rule is that a brand is passed explicitly at the one call site that
// means it and is NEVER reachable by pattern. A glyphFor rule keyed on "coffee"
// would put another company's cup on anything mentioning coffee, and a rule on
// "crypto" would put the Bitcoin symbol on settings that have nothing to do
// with it. See gen_glyphs.py's IconGithub entry for the full reasoning.
//
// They also travel with lib/donate.ts rather than with the action glyphs,
// because that is what they belong to: the day the donation routes change,
// these change with them, and nothing else in the app reads this file.
//
// Attribution, required by the licences:
//   Simple Icons (https://simpleicons.org) - CC0 1.0 Universal: Bitcoin,
//   Ethereum, Tether, Solana, Binance, XRP, Sui, Buy Me a Coffee, PayPal.
//   cryptocurrency-icons (https://github.com/spothq/cryptocurrency-icons) -
//   MIT: USD Coin and XRP. Copyright (c) 2018 Christopher Downer.
//
// Every mark carries its OWN brand colour (jdp, 2026-09-10: "Die Logos der
// Kryptowährungen sollen die original-logos sein mit deren original Farben"),
// REVERSING what this file did before, which was to fill every mark with
// `currentColor` so it took the ink of its tile.
//
// The old reasoning is preserved here because it was not wrong, only
// outweighed: coloured coin logos are the only coloured drawings in this
// interface, and on a filled tile a brand colour sits on a background it was
// never designed against. What outweighs it is recognition. A donor scanning
// eight tiles for the coin they hold finds Bitcoin's orange and Tether's green
// faster than they read eight three-letter tickers, and that is the entire job
// of this grid. KnightLoader's browser tiles already settled the same question
// the same way: vendor marks keep their own colours through the hover, and
// only the surface and the text change.
//
// XRP is the one mark that cannot simply take its brand colour, because that
// colour is near-black and would vanish on a dark surface. It gets a light
// variant for the dark theme - the same pair the brand itself publishes -
// rather than a compromise grey that is nobody's colour.
// ---------------------------------------------------------------------------
import type { ReactNode } from "react";

function Mark({
  box,
  d,
  size = 16,
  color,
}: {
  box: string;
  d: string;
  size?: number;
  /** The brand's own colour. Omitted for the marks that still take the ink of
   *  whatever they sit on - the two donation-route buttons, which sit in a row
   *  of ordinary buttons and would be the only coloured things in it. */
  color?: string;
}) {
  return (
    <svg
      width={size}
      height={size}
      viewBox={box}
      fill={color ?? "currentColor"}
      className="shrink-0"
      aria-hidden="true"
    >
      <path d={d} />
    </svg>
  );
}

const B24 = "0 0 24 24";

/**
 * Each coin's own colour, as the brand publishes it.
 *
 * All but one are a literal, because a brand colour that reads on both a light
 * and a dark ground is the whole point of having one. XRP is the exception:
 * its mark is black, so it takes a CSS variable the theme switches for it.
 */
const PATHS: Record<string, { box: string; d: string; color: string }> = {
  btc: {
    box: B24,
    d: "M23.638 14.904c-1.602 6.43-8.113 10.34-14.542 8.736C2.67 22.05-1.244 15.525.362 9.105 1.962 2.67 8.475-1.243 14.9.358c6.43 1.605 10.342 8.115 8.738 14.548v-.002zm-6.35-4.613c.24-1.59-.974-2.45-2.64-3.03l.54-2.153-1.315-.33-.525 2.107c-.345-.087-.705-.167-1.064-.25l.526-2.127-1.32-.33-.54 2.165c-.285-.067-.565-.132-.84-.2l-1.815-.45-.35 1.407s.975.225.955.236c.535.136.63.486.615.766l-1.477 5.92c-.075.166-.24.406-.614.314.015.02-.96-.24-.96-.24l-.66 1.51 1.71.426.93.242-.54 2.19 1.32.327.54-2.17c.36.1.705.19 1.05.273l-.51 2.154 1.32.33.545-2.19c2.24.427 3.93.257 4.64-1.774.57-1.637-.03-2.58-1.217-3.196.854-.193 1.5-.76 1.68-1.93h.01zm-3.01 4.22c-.404 1.64-3.157.75-4.05.53l.72-2.9c.896.23 3.757.67 3.33 2.37zm.41-4.24c-.37 1.49-2.662.735-3.405.55l.654-2.64c.744.18 3.137.524 2.75 2.084v.006z",
    color: "#F7931A",
  },
  eth: {
    box: B24,
    d: "M11.944 17.97L4.58 13.62 11.943 24l7.37-10.38-7.372 4.35h.003zM12.056 0L4.69 12.223l7.365 4.354 7.365-4.35L12.056 0z",
    color: "#627EEA",
  },
  usdt: {
    box: B24,
    d: "M18.7538 10.5176c0 .6251-2.2379 1.1483-5.2381 1.2812l.0028.0007c-.0848.0064-.5233.0325-1.5012.0325-.7778 0-1.33-.0233-1.5237-.0325-3.0059-.1322-5.2495-.6555-5.2495-1.2819s2.2436-1.149 5.2495-1.2834v2.0442c.1965.0142.7594.0474 1.5372.0474.9334 0 1.4008-.0389 1.4849-.0466V9.2356c2.9994.1337 5.2381.657 5.2381 1.282zm5.19.5466L12.1248 22.389a.1803.1803 0 0 1-.2496 0L.0562 11.0635a.1781.1781 0 0 1-.0382-.2079l4.3762-9.1921a.1767.1767 0 0 1 .1626-.1026h14.8878a.1768.1768 0 0 1 .1612.1032l4.3762 9.1922a.1782.1782 0 0 1-.0382.2079zm-4.478-.4038c0-.8068-2.5515-1.4799-5.9473-1.6369V7.195h4.186V4.4055H6.3076V7.195h4.1852v1.8286c-3.4018.1562-5.9601.83-5.9601 1.6376 0 .8075 2.5583 1.4806 5.9601 1.6376v5.8618h3.025v-5.8639c3.394-.1563 5.948-.8295 5.948-1.6363z",
    color: "#26A17B",
  },
  usdc: {
    box: "0 0 32 32",
    d: "M16 0c8.837 0 16 7.163 16 16s-7.163 16-16 16S0 24.837 0 16 7.163 0 16 0zm3.352 5.56c-.244-.12-.488 0-.548.243-.061.061-.061.122-.061.243v.85l.01.104a.86.86 0 00.355.503c4.754 1.7 7.192 6.98 5.424 11.653-.914 2.55-2.925 4.491-5.424 5.402-.244.121-.365.303-.365.607v.85l.005.088a.45.45 0 00.36.397c.061 0 .183 0 .244-.06a10.895 10.895 0 007.13-13.717c-1.096-3.46-3.778-6.07-7.13-7.162zm-6.46-.06c-.061 0-.183 0-.244.06a10.895 10.895 0 00-7.13 13.717c1.096 3.4 3.717 6.01 7.13 7.102.244.121.488 0 .548-.243.061-.06.061-.122.061-.243v-.85l-.01-.08c-.042-.169-.199-.362-.355-.466-4.754-1.7-7.192-6.98-5.424-11.653.914-2.55 2.925-4.491 5.424-5.402.244-.121.365-.303.365-.607v-.85l-.005-.088a.45.45 0 00-.36-.397zm3.535 3.156h-.915l-.088.008c-.2.04-.346.212-.4.478v1.396l-.207.032c-1.708.304-2.778 1.483-2.778 2.942 0 2.002 1.218 2.791 3.778 3.095 1.707.303 2.255.668 2.255 1.639 0 .97-.853 1.638-2.011 1.638-1.585 0-2.133-.667-2.316-1.578-.06-.242-.244-.364-.427-.364h-1.036l-.079.007a.413.413 0 00-.347.418v.06l.033.18c.29 1.424 1.266 2.443 3.197 2.734v1.457l.008.088c.04.198.213.344.48.397h.914l.088-.008c.2-.04.346-.212.4-.477V21.34l.207-.04c1.713-.362 2.84-1.601 2.84-3.177 0-2.124-1.28-2.852-3.84-3.156-1.829-.243-2.194-.728-2.194-1.578 0-.85.61-1.396 1.828-1.396 1.097 0 1.707.364 2.011 1.275a.458.458 0 00.427.303h.975l.079-.006a.413.413 0 00.348-.419v-.06l-.037-.173a3.04 3.04 0 00-2.706-2.316V9.142l-.008-.088c-.04-.199-.213-.345-.48-.398z",
    color: "#2775CA",
  },
  bnb: {
    box: B24,
    d: "M16.624 13.9202l2.7175 2.7154-7.353 7.353-7.353-7.352 2.7175-2.7164 4.6355 4.6595 4.6356-4.6595zm4.6366-4.6366L24 12l-2.7154 2.7164L18.5682 12l2.6924-2.7164zm-9.272.001l2.7163 2.6914-2.7164 2.7174v-.001L9.2721 12l2.7164-2.7154zm-9.2722-.001L5.4088 12l-2.6914 2.6924L0 12l2.7164-2.7164zM11.9885.0115l7.353 7.329-2.7174 2.7154-4.6356-4.6356-4.6355 4.6595-2.7174-2.7154 7.353-7.353z",
    color: "#F3BA2F",
  },
  sol: {
    box: B24,
    d: "m23.8764 18.0313-3.962 4.1393a.9201.9201 0 0 1-.306.2106.9407.9407 0 0 1-.367.0742H.4599a.4689.4689 0 0 1-.2522-.0733.4513.4513 0 0 1-.1696-.1962.4375.4375 0 0 1-.0314-.2545.4438.4438 0 0 1 .117-.2298l3.9649-4.1393a.92.92 0 0 1 .3052-.2102.9407.9407 0 0 1 .3658-.0746H23.54a.4692.4692 0 0 1 .2523.0734.4531.4531 0 0 1 .1697.196.438.438 0 0 1 .0313.2547.4442.4442 0 0 1-.1169.2297zm-3.962-8.3355a.9202.9202 0 0 0-.306-.2106.941.941 0 0 0-.367-.0742H.4599a.4687.4687 0 0 0-.2522.0734.4513.4513 0 0 0-.1696.1961.4376.4376 0 0 0-.0314.2546.444.444 0 0 0 .117.2297l3.9649 4.1394a.9204.9204 0 0 0 .3052.2102c.1154.049.24.0744.3658.0746H23.54a.469.469 0 0 0 .2523-.0734.453.453 0 0 0 .1697-.1961.4382.4382 0 0 0 .0313-.2546.4444.4444 0 0 0-.1169-.2297zM.46 6.7225h18.7815a.9411.9411 0 0 0 .367-.0742.9202.9202 0 0 0 .306-.2106l3.962-4.1394a.4442.4442 0 0 0 .117-.2297.4378.4378 0 0 0-.0314-.2546.453.453 0 0 0-.1697-.196.469.469 0 0 0-.2523-.0734H4.7596a.941.941 0 0 0-.3658.0745.9203.9203 0 0 0-.3052.2102L.1246 5.9687a.4438.4438 0 0 0-.1169.2295.4375.4375 0 0 0 .0312.2544.4512.4512 0 0 0 .1692.196.4689.4689 0 0 0 .2518.0739z",
    color: "#9945FF",
  },
  sui: {
    box: B24,
    d: "M17.636 10.009a7.16 7.16 0 0 1 1.565 4.474 7.2 7.2 0 0 1-1.608 4.53l-.087.106-.023-.135a7 7 0 0 0-.07-.349c-.502-2.21-2.142-4.106-4.84-5.642-1.823-1.034-2.866-2.278-3.14-3.693-.177-.915-.046-1.834.209-2.62.254-.787.631-1.446.953-1.843l1.05-1.284a.46.46 0 0 1 .713 0l5.28 6.456zm1.66-1.283L12.26.123a.336.336 0 0 0-.52 0L4.704 8.726l-.023.029a9.33 9.33 0 0 0-2.07 5.872C2.612 19.803 6.816 24 12 24s9.388-4.197 9.388-9.373a9.32 9.32 0 0 0-2.07-5.871zM6.389 9.981l.63-.77.018.142q.023.17.055.34c.408 2.136 1.862 3.917 4.294 5.297 2.114 1.203 3.345 2.586 3.7 4.103a5.3 5.3 0 0 1 .109 1.801l-.004.034-.03.014A7.2 7.2 0 0 1 12 21.67c-3.976 0-7.2-3.218-7.2-7.188 0-1.705.594-3.27 1.587-4.503z",
    color: "#4DA2FF",
  },
  xrp: {
    // The current XRP mark, not Ripple's old wave lines: those are the
    // COMPANY's former logo and read as a set of brackets at 22px, which
    // is exactly how it was reported ("das logo von XRP passt nicht").
    box: "0 0 32 32",
    d: "M16 32C7.163 32 0 24.837 0 16S7.163 0 16 0s16 7.163 16 16-7.163 16-16 16zm7.07-24l-4.574 4.523a3.556 3.556 0 01-4.996 0L8.93 8H6.035l6.02 5.957a5.621 5.621 0 007.89 0L25.961 8h-2.89zM8.895 24.563L13.504 20a3.556 3.556 0 014.996 0l4.605 4.563H26l-6.055-5.993a5.621 5.621 0 00-7.89 0L6 24.562h2.895z",
    // A CSS variable rather than a literal, because this is the one mark whose
    // colour has to change with the theme and a value read in JavaScript would
    // not follow a theme switch without a subscription. index.css defines it
    // for all three theme states beside every other colour token.
    color: "var(--coin-xrp)",
  },
};

/**
 * PayPal's own mark, for the button that opens a PayPal.Me page.
 *
 * The same rule as every brand here: passed explicitly at the one call site
 * that means it, never reachable by pattern.
 */
export function IconPayPal({ size = 16 }: { size?: number }): ReactNode {
  return (
    <Mark
      box={B24}
      size={size}
      d="M15.607 4.653H8.941L6.645 19.251H1.82L4.862 0h7.995c3.754 0 6.375 2.294 6.473 5.513-.648-.478-2.105-.86-3.722-.86m6.57 5.546c0 3.41-3.01 6.853-6.958 6.853h-2.493L11.595 24H6.74l1.845-11.538h3.592c4.208 0 7.346-3.634 7.153-6.949a5.24 5.24 0 0 1 2.848 4.686M9.653 5.546h6.408c.907 0 1.942.222 2.363.541-.195 2.741-2.655 5.483-6.441 5.483H8.714Z"
    />
  );
}

/** Whether a coin id has a mark here. The test beside lib/donate.ts holds
 *  every offered coin to having one, so no tile ships as a bare ticker. */
export function hasCoinMark(coin: string): boolean {
  return coin in PATHS;
}

/** A coin's own mark, by the id lib/donate.ts gives it. */
export function CoinMark({ coin, size = 16 }: { coin: string; size?: number }): ReactNode {
  const mark = PATHS[coin];
  // Undefined rather than a placeholder: a tile still carries its ticker, and
  // a symbol that means nothing is worse than none. The test beside donate.ts
  // holds every offered coin to having one, so this is a backstop, not a plan.
  if (!mark) return null;
  return <Mark box={mark.box} d={mark.d} size={size} color={mark.color} />;
}

/**
 * Buy Me a Coffee's own mark, for the button that opens their page.
 *
 * The same rule as the GitHub mark on the repository button: a brand is
 * recognised faster than a word is read, and it belongs only on the control
 * that actually goes there.
 */
export function IconBuyMeACoffee({ size = 16 }: { size?: number }): ReactNode {
  return (
    <Mark
      box={B24}
      size={size}
      d="M20.216 6.415l-.132-.666c-.119-.598-.388-1.163-1.001-1.379-.197-.069-.42-.098-.57-.241-.152-.143-.196-.366-.231-.572-.065-.378-.125-.756-.192-1.133-.057-.325-.102-.69-.25-.987-.195-.4-.597-.634-.996-.788a5.723 5.723 0 00-.626-.194c-1-.263-2.05-.36-3.077-.416a25.834 25.834 0 00-3.7.062c-.915.083-1.88.184-2.75.5-.318.116-.646.256-.888.501-.297.302-.393.77-.177 1.146.154.267.415.456.692.58.36.162.737.284 1.123.366 1.075.238 2.189.331 3.287.37 1.218.05 2.437.01 3.65-.118.299-.033.598-.073.896-.119.352-.054.578-.513.474-.834-.124-.383-.457-.531-.834-.473-.466.074-.96.108-1.382.146-1.177.08-2.358.082-3.536.006a22.228 22.228 0 01-1.157-.107c-.086-.01-.18-.025-.258-.036-.243-.036-.484-.08-.724-.13-.111-.027-.111-.185 0-.212h.005c.277-.06.557-.108.838-.147h.002c.131-.009.263-.032.394-.048a25.076 25.076 0 013.426-.12c.674.019 1.347.067 2.017.144l.228.031c.267.04.533.088.798.145.392.085.895.113 1.07.542.055.137.08.288.111.431l.319 1.484a.237.237 0 01-.199.284h-.003c-.037.006-.075.01-.112.015a36.704 36.704 0 01-4.743.295 37.059 37.059 0 01-4.699-.304c-.14-.017-.293-.042-.417-.06-.326-.048-.649-.108-.973-.161-.393-.065-.768-.032-1.123.161-.29.16-.527.404-.675.701-.154.316-.199.66-.267 1-.069.34-.176.707-.135 1.056.087.753.613 1.365 1.37 1.502a39.69 39.69 0 0011.343.376.483.483 0 01.535.53l-.071.697-1.018 9.907c-.041.41-.047.832-.125 1.237-.122.637-.553 1.028-1.182 1.171-.577.131-1.165.2-1.756.205-.656.004-1.31-.025-1.966-.022-.699.004-1.556-.06-2.095-.58-.475-.458-.54-1.174-.605-1.793l-.731-7.013-.322-3.094c-.037-.351-.286-.695-.678-.678-.336.015-.718.3-.678.679l.228 2.185.949 9.112c.147 1.344 1.174 2.068 2.446 2.272.742.12 1.503.144 2.257.156.966.016 1.942.053 2.892-.122 1.408-.258 2.465-1.198 2.616-2.657.34-3.332.683-6.663 1.024-9.995l.215-2.087a.484.484 0 01.39-.426c.402-.078.787-.212 1.074-.518.455-.488.546-1.124.385-1.766zm-1.478.772c-.145.137-.363.201-.578.233-2.416.359-4.866.54-7.308.46-1.748-.06-3.477-.254-5.207-.498-.17-.024-.353-.055-.47-.18-.22-.236-.111-.71-.054-.995.052-.26.152-.609.463-.646.484-.057 1.046.148 1.526.22.577.088 1.156.159 1.737.212 2.48.226 5.002.19 7.472-.14.45-.06.899-.13 1.345-.21.399-.072.84-.206 1.08.206.166.281.188.657.162.974a.544.544 0 01-.169.364zm-6.159 3.9c-.862.37-1.84.788-3.109.788a5.884 5.884 0 01-1.569-.217l.877 9.004c.065.78.717 1.38 1.5 1.38 0 0 1.243.065 1.658.065.447 0 1.786-.065 1.786-.065.783 0 1.434-.6 1.499-1.38l.94-9.95a3.996 3.996 0 00-1.322-.238c-.826 0-1.491.284-2.26.613z"
    />
  );
}

/**
 * Bitcoin's mark, on the button that opens the crypto window.
 *
 * jdp asked for it by name (2026-09-10). It is the one symbol that reads as
 * "crypto" to somebody who has never held any - the way a floppy disk still
 * reads as "save" - and that is worth more here than the accuracy of naming
 * one chain out of five. What keeps the naming honest is the window itself:
 * the first thing in it is a grid of every coin on offer, each with its own
 * mark, so nobody gets as far as an address believing Bitcoin is the only
 * option. This replaced a neutral wallet drawing, which was correct and said
 * nothing.
 */
export function IconBitcoin({ size = 16 }: { size?: number }): ReactNode {
  return <CoinMark coin="btc" size={size} />;
}
