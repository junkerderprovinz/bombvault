...
export function SelectionTree({
  ...
  // component. Everything above the render (listings, expansion, focus,
  // classification) is mode-blind; only renderSpec's row JSX branches below.
        ...
        >
          <span className="mt-0.5 w-4 shrink-0 flex items-center justify-center" aria-hidden="true">
            {!touch && spec.expandable && (
              ...
              // separate label to translate or double-fire. Touch mode drops
              // the glyph (the affordance moves into the labelled chevron
              // button at the row end — an unlabeled twin would be a second
              // expansion control to keep in sync) but KEEPS the span: it is
              // the 16px alignment register every checkbox in the column
              // shares.
              <svg
                width="10"
                height="10"
          ...
          {/* aria-hidden on the input (review WR-02): the treeitem's
              ...
              Touch mode (Pitfall 1) makes the input PURELY presentational:
              pointer-events-none lets a tap on the glyph fall through to the
              row (the one toggle surface), and dropping onChange with the
              input readOnly removes the native toggle — the desktop
              combination of stopPropagation + native checkbox toggle would
              otherwise double-fire (two toggles = no-op) or strand a dead
              zone inside the "full-row target". The state it paints comes
              from the same (includes, exclusions) mirror as aria-checked. */}
          <input
            ref={boxRef}
            type="checkbox"
            aria-hidden="true"
