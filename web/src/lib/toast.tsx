...
// resume measurements, multi-toast stacking, keyboard dismissal, and the
// non-blocking click-through requirement.
// ---------------------------------------------------------------------------
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const push = useCallback(
    (message: string, severity: ToastSeverity = "success") => {
      ...
      setToasts((list) => addToast(list, { id, message, severity }, Date.now()));
  ...
  // mid-hover) and freeze remainingMs at the pure engine's own math.
  ...
  // Hover/focus leaves a toast: restart the timer from whatever pauseToast
  // froze — NOT the full TOAST_DURATION_MS again (that would be the exact
  // "resets to full duration" bug the spec calls out by name).
  ...
  return (
    <ToastContext.Provider value={{ push, quiet, setQuiet }}>
      {children}
      {viewport}
