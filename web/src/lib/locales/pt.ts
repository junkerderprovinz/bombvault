...
const pt: Partial<Translations> = {
  ...
  "repos.itemHint": "Para onde vão as cópias deste item. Deixa no repositório do domínio a não ser que este item precise de um próprio; a razão habitual é um item grande e que muda pouco do qual só se quer uma cópia fora do local. A lista contém o que criaste nas Definições. A replicação externa não volta a copiar um repositório remoto nem um marcado como já fora do local.",
  ...
  "repos.offsite": "Já fora do local",
  "repos.offsiteHint": "Para uma partilha noutra máquina, como um NAS usado como armazenamento a frio. A replicação externa deixa então este repositório de fora, tal como um remoto, e as suas cópias não são copiadas uma segunda vez. Deixa desligado para um disco ou partilha desta máquina, que continua a precisar da cópia fora do local.",
  "repos.offsiteRemote": "fora do local",
  "repos.offsiteRemoteHint": "Um repositório remoto já está fora do local, por isso a replicação externa nunca o volta a copiar.",
