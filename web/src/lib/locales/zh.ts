...
const zh: Partial<Translations> = {
  ...
  "repos.itemHint": "这一项的备份去往何处。除非这一项确实需要专属的，否则就留在该域的存储库上。常见的理由是一个很大、极少变动的项目，只需要一份异地副本。列表里是你在设置中建立的那些。异地复制不会把远程存储库或已标记为已在异地的存储库复制第二次。",
  ...
  "repos.offsite": "已在异地",
  "repos.offsiteHint": "适用于另一台机器上的共享，例如用作冷存储的 NAS。开启后，异地复制会像对待远程存储库一样跳过这个存储库，其中的备份不会被复制第二次。本机上的磁盘或共享请保持关闭，它们仍然需要异地副本。",
  "repos.offsiteRemote": "异地",
  "repos.offsiteRemoteHint": "远程存储库本来就在异地，所以异地复制永远不会再复制它。",
  "repos.inUse": "{n} 项在用",
  "repos.unused": "未使用",
