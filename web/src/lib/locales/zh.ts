...
const zh: Partial<Translations> = {
  ...
  "settings.offsiteHint": "每次本地备份成功后，再用 restic copy 复制到第二个仓库。填写远程仓库（rest:http://host:8000/repo、s3:…）或本地子路径；留空则停用。本地备份仍为主备份。",
  "source.label": "来源：",
  "source.local": "本地",
  "source.offsite": "异地",
  ...
  "cloud.hint": "用于远程 restic 后端的凭据，无需 rclone。保存后，将备份路径设置为远程仓库，例如 s3:s3.amazonaws.com/bucket/path、rest:http://host:8000/repo 或 sftp:user@host:/repo。密钥以加密方式存储，不会再次显示。",
  ...
  "ransomware.replicationPaused": "异地复制已暂停：请确认存放位置",
  ...
  "pull.repoLocationHint": "来源实例的仓库：rest:、s3:、sftp:，或主机挂载下的子路径。rclone: 位置会被拒绝，因为 rclone 会用本实例的远程配置去访问它。",
  "pull.appKey": "来源 APP_KEY",
  "pull.appKeyHint": "对方实例的密钥，64 位小写十六进制字符。它就是解锁对方仓库的钥匙，在此加密保存，且不再显示。",
  "pull.credsNone": "无 (来源无需登录)",
  "pull.credsHint": "来源仓库所在存储的登录信息，适用于 s3: 或带密码的 rest: 服务器。拉取从不使用本机自己的凭据，因此需要登录却未在此选择的来源会被拒绝。",
  "pull.domain": "备份种类",
  ...
  "repos.location": "位置",
  "repos.locationPlaceholder": "backups/cold 或 s3:s3.eu-central-003.backblazeb2.com/bucket/path",
  "repos.locationHint": "宿主挂载点下的一个文件夹，或者像 s3:...、sftp:...、rest:...、rclone:... 这样的远程 restic 存储库。远程是直接写入的，中间没有本地副本。",
  ...
  "placementCode.copyRuleTaken": "该名称已有自己的复制规则。",
  ...
  "placement.titleHint": "此项目的备份写入何处，之后又复制到何处。位置自首次备份起固定，副本可以随时更改。",
  "placement.unreadable": "无法读取存放位置",
  "placement.noCopyNow": "目前没有副本，已关闭：{targets}。勾选一个目标或选择本地。",
  "placement.followsDefault": "跟随默认",
  "placement.ownCopies": "自有副本",
  "placement.homeSet": "位置已设置：{home}",
  "placement.copiesFollow": "副本跟随默认",
  "placement.reset": "重置",
  ...
  // Timeline
  "timeline.deleteAsk": "在此处删除此备份：{list}？",
  ...
  "timeline.onlyHere": "仅此处",
  "timeline.incomplete": "不完整",
  "timeline.incompleteHint": "此次运行的并非所有磁盘都在这里，因此不会选择此位置进行恢复。",
  "timeline.snapshotMissing": "{place} 不再有此备份。是否从 {next} 恢复？",
  "timeline.stackDirMissing": "项目文件夹不在 {place} 中。是否使用 {home} 的状态？",
  // Direct repository
  ...
  "placementDefaults.homeOff": "{home} 已关闭。没有位置的项目在它重新开启或默认值更改之前不会被备份。",
  "placementDefaults.homeMissing": "默认值指向一个已不存在的仓库。没有位置的项目不会被备份。",
  "placementDefaults.confirmHome": "{domain} 中的新项目在首次备份时采用 {home}。已有位置的项目保持不变。",
  "placementDefaults.dropAsk": "{target} 不再接收的项目和项目文件夹：{n}。留在那里的副本：{copies}。",
  "placementDefaults.dropAskUnknown": "{target} 不再接收的项目和项目文件夹：{n}。{target} 从未在此域中列出过，因此目前还不知道那里会留下多少副本。",
  "placementDefaults.addAsk": "{target} 从现在起接收的项目和项目文件夹：{n}。下次运行时上传的快照：最多 {snapshots} 个。",
  "placementDefaults.openTakeHome": "在首次备份时采用 {home} 的无位置项目：{n}。",
  "placementDefaults.apply": "应用于没有备份的项目",
  "placementDefaults.applyAsk": "以下项目的位置和副本将恢复为默认值：{n}。",
  "placementDefaults.applyLoses": "因此失去自己选择的项目：{list}",
  "placementDefaults.applyKeptBackups": "保持不变，因为它们已有备份：{list}",
  "placementDefaults.applyKeptUnreadable": "保持不变，因为无法读取其位置：{list}",
  "placementDefaults.applyKeptChanged": "在此期间发生变化，未被触碰：{list}",
  "placementDefaults.applyNone": "没有备份的项目中没有一个与默认值不同。",
  "placementDefaults.applyBusy": "备份正在进行。完成后请重新应用。",
  "placementDefaults.paused": "重建后异地已暂停",
  "placementDefaults.pausedHint": "规则和默认值随旧配置一起丢失。在此默认值被确认之前，不会从 {domain} 向异地复制任何内容。带卡片的项目可以先设为本地。",
  "placementDefaults.confirm": "确认默认值",
  "placementDefaults.confirmAsk": "下次运行时，将复制以下内容：",
  "placementDefaults.unmatched": "备份中没有对应条目的名称。已勾选的将被排除在外：",
  "placementDefaults.unmatchedLine": "{name}，快照：{n}",
  "placementDefaults.confirmed": "默认值已确认。异地副本将在下次运行时恢复。",
  ...
  "placement.older": "{target} 处的较旧副本：{n} 个，最后一次看到 {date}",
  "placement.noCopyYet": "{place}：尚无副本",
  "placement.notListedYet": "{place}：尚未列出",

  // Removal
  "offsiteRemoval.delete": "在 {target} 删除",
  "offsiteRemoval.ask": "删除 {target} 中 {name} 的每一个副本？副本数：{n}。",
  "offsiteRemoval.onlyThere": "这些快照不存在于其他任何地方，此后将永久消失：",
