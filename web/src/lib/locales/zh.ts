...
const zh: Partial<Translations> = {
  ...
  "dbdump.introNotice": "BombVault 会在每次备份前转储它识别出的数据库，作为各自独立的还原点。这里识别出的数量：{count}，第一个是 {name}。你可以在容器卡片上逐个关闭，或在设置里对所有容器关闭。",
  ...
  "dbdump.importStopsOne": "导入期间，BombVault 也会停止 {app}。完成后会重新启动它，除非导入失败且旧数据无法放回。",
  "dbdump.importStopsMany": "导入期间，BombVault 也会停止以下应用：{apps}。完成后会重新启动它们，除非导入失败且旧数据无法放回。",
  ...
  "recovery.foreignDbDumps": "本仓库中的数据库转储：{count} 个。按恢复包所述，用 restic 命令行取出其中一个。",
  ...
  "recovery.dumpOnlySkipped": "只以转储形式保存的数据库在这里被略过：{count}。请在它们自己的列表里还原并导入。",
