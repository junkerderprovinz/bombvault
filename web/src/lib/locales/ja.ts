...
const ja: Partial<Translations> = {
  ...
  "dbdump.introNotice": "BombVault はバックアップのたびに、認識したデータベースを独立した復元ポイントとしてダンプします。ここで認識した数: {count}、最初は {name} です。コンテナごとにそのカードで、または設定ですべてのコンテナについて無効にできます。",
  ...
  "dbdump.importStopsOne": "取り込みの間、BombVault は {app} も停止します。取り込みが失敗して元のデータを戻せなかった場合を除き、終わったら再び起動します。",
  "dbdump.importStopsMany": "取り込みの間、BombVault は次のアプリも停止します: {apps}。取り込みが失敗して元のデータを戻せなかった場合を除き、終わったら再び起動します。",
  ...
  "recovery.foreignDbDumps": "このリポジトリのデータベースダンプ: {count} 件。復旧キットの説明どおり restic のコマンドラインで取り出せます。",
  ...
  "recovery.dumpOnlySkipped": "ダンプとしてだけ保存されているデータベースはここでは対象外です: {count}。専用の一覧から復元して取り込んでください。",
