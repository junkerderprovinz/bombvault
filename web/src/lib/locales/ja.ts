...
const ja: Partial<Translations> = {
  ...
  "coverage.hint": "このサーバー上で、自動バックアップの対象になっていないものすべてです。上の保護ステータスはスケジュールされたバックアップが時間どおりに実行されたかを示しますが、こちらはそもそもスケジュールされていないものを示します。これは他のどこにも表れない抜けです。オフにしたバックアップドメインは意図的な判断なので、ここには数えません。",
  ...
  "settings.everythingDuplicateWarning": "このスケジュールと各ドメイン自身のスケジュールが両方とも有効な間、コンテナ、VM、フラッシュ、フォルダー、セルフバックアップは 2 回バックアップされます。1 回はここで、もう 1 回はそれぞれのスケジュールで実行されます。重複させたくないドメインはオフにしてください。",
