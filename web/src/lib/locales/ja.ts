...
const ja: Partial<Translations> = {
  ...
  "files.empty": "ファイルセットはまだありません。フォルダーを追加してください — 共有、ドキュメント、写真など、マウント配下のあらゆるもの — BombVault が他のものと同じように保護します：スケジュール、オフサイトコピー、整合性チェック、復元。別のファイルバックアップツールは不要です。",
  "files.addSet": "ファイルセットを追加",
  "files.editSet": "ファイルセットを編集",
  "files.name": "名前",
  "files.path": "フォルダー",
  ...
  "files.deleteSetConfirm": "このファイルセットを一覧から削除しますか？バックアップは削除されず、後で再発見できます。",
  "files.deleteBackupsConfirm": "このファイルセットのバックアップをすべて削除しますか？スナップショットは完全に削除され、リポジトリが整理され、セットは忘れられます。元に戻せません。",
