...
const ja: Partial<Translations> = {
  ...
  "repos.itemHint": "この項目のバックアップの行き先です。この項目に専用のものが必要でない限り、ドメインのリポジトリのままにしてください。よくある理由は、めったに変わらない大きな項目で、オフサイトの控えだけが欲しい場合です。一覧には設定で作成したものが並びます。リモートのリポジトリや、既にオフサイトと印を付けたリポジトリを、オフサイト複製が二度コピーすることはありません。",
  ...
  "repos.offsite": "既にオフサイト",
  "repos.offsiteHint": "別のマシン上の共有向けです。たとえばコールドストレージとして使う NAS です。オンにするとオフサイト複製はリモートのリポジトリと同じくこのリポジトリを対象から外し、そのバックアップを二度コピーしません。このマシン上のディスクや共有ではオフのままにしてください。そちらには今もオフサイトの控えが必要です。",
  "repos.offsiteRemote": "オフサイト",
  "repos.offsiteRemoteHint": "リモートのリポジトリは既にオフサイトにあるため、オフサイト複製が再びコピーすることはありません。",
  "repos.inUse": "{n} 件が使用中",
  "repos.unused": "未使用",
