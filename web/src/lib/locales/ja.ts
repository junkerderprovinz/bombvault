...
const ja: Partial<Translations> = {
  ...
  "settings.offsiteHint": "ローカルバックアップが成功するたびに、restic copy で2つ目のリポジトリにも複製します。リモート（rest:http://host:8000/repo、s3:…）またはローカルのサブパスを入力してください。無効にするには空欄のままにします。ローカルバックアップが主のままです。",
  "source.label": "ソース:",
  "source.local": "ローカル",
  ...
  "cloud.hint": "rclone なしのリモート restic バックエンド用の認証情報。保存後、バックアップパスをリモートリポジトリに設定します（例: s3:s3.amazonaws.com/bucket/path、rest:http://host:8000/repo、sftp:user@host:/repo）。シークレットは暗号化して保存され、再表示されません。",
  ...
  "pull.repoLocationHint": "取得元インスタンスのリポジトリ: rest:、s3:、sftp:、あるいはホストマウント下のサブパス。rclone: の場所は拒否されます。rclone ではこのインスタンス側のリモート設定で接続してしまうためです。",
  ...
  "pull.credsHint": "取得元のリポジトリが置かれているストレージへのログイン情報です。s3:、またはパスワード付きの rest: サーバーが対象です。取得はこの機器の資格情報を決して使わないため、ログインが必要なのにここで何も選ばれていない取得元は拒否されます。",
  ...
  "repos.location": "場所",
  "repos.locationPlaceholder": "backups/cold または s3:s3.eu-central-003.backblazeb2.com/bucket/path",
  "repos.locationHint": "ホストマウント配下のフォルダー、または s3:...、sftp:...、rest:...、rclone:... のようなリモートの restic リポジトリです。リモートには途中のローカル控えなしで直接書き込みます。",
  ...
  "repos.offPremises": "敷地外",
  "repos.offPremisesHint": "カード上のサイト数と3-2-1では独立したサイトとして数えられます。コピーには影響しません。同じ建物内の rest-server ではこれをオフにしてください。",
  ...
  "placement.titleHint": "この項目のバックアップがどこに書き込まれ、その後どこにコピーされるか。保存先は最初のバックアップ以降固定され、コピー先はいつでも変更できます。",
  "placement.unreadable": "配置を読み取れませんでした",
  "placement.noCopyNow": "現在コピーはありません、オフ: {targets}。ターゲットにチェックを入れるか、ローカルを選んでください。",
  "placement.followsDefault": "既定に従う",
  "placement.ownCopies": "独自のコピー",
  "placement.homeSet": "保存先を設定: {home}",
  "placement.copiesFollow": "コピーは既定に従う",
  "placement.reset": "リセット",
  ...
  // Timeline
  "timeline.deleteAsk": "このバックアップをここで削除しますか: {list}?",
  ...
  "timeline.onlyHere": "ここのみ",
  "timeline.incomplete": "不完全",
  "timeline.incompleteHint": "この実行のすべてのディスクがここにあるわけではないため、この場所は復元に選ばれません。",
  "timeline.snapshotMissing": "{place} にはこのバックアップがもうありません。{next} から復元しますか?",
  "timeline.stackDirMissing": "プロジェクトフォルダーが {place} にありません。{home} の状態を使いますか?",
  // Direct repository
  ...
  "placementDefaults.homeOff": "{home} はオフになっています。保存先がない項目は、再度オンになるか既定が変わるまでバックアップされません。",
  "placementDefaults.homeMissing": "既定はもう存在しないリポジトリを指しています。保存先がない項目はバックアップされません。",
  "placementDefaults.confirmHome": "{domain} の新しい項目は、最初のバックアップ時に {home} を引き継ぎます。保存先がある項目はそれを維持します。",
  "placementDefaults.dropAsk": "{target} がもう受け取らなくなる項目とプロジェクトフォルダー: {n}。そこに残るコピー: {copies}。",
  "placementDefaults.dropAskUnknown": "{target} がもう受け取らなくなる項目とプロジェクトフォルダー: {n}。{target} はこのドメイン向けに一度も登録されたことがないため、そこに何個のコピーが残るかはまだわかりません。",
  "placementDefaults.addAsk": "{target} が今後受け取る項目とプロジェクトフォルダー: {n}。次回実行時にアップロードされるスナップショット: 最大{snapshots}。",
  "placementDefaults.openTakeHome": "最初のバックアップ時に {home} を引き継ぐ、保存先のない項目: {n}。",
  "placementDefaults.apply": "バックアップのない項目に適用",
  "placementDefaults.applyAsk": "これらの項目について、保存先とコピーが既定に戻ります: {n}。",
  "placementDefaults.applyLoses": "その際に独自の選択が失われる項目: {list}",
  "placementDefaults.applyKeptBackups": "そのままにされる項目、バックアップがあります: {list}",
  "placementDefaults.applyKeptUnreadable": "そのままにされる項目、保存先を読み取れませんでした: {list}",
  "placementDefaults.applyKeptChanged": "その間に変更され、手を加えられなかった項目: {list}",
  "placementDefaults.applyNone": "バックアップのない項目に、既定と異なるものはありません。",
  "placementDefaults.applyBusy": "バックアップが実行中です。終了後にもう一度適用してください。",
  "placementDefaults.paused": "再構築後、オフサイトは一時停止中",
  "placementDefaults.pausedHint": "古い設定とともにルールと既定値が失われました。この既定が確認されるまで、{domain} からは何もオフサイトにコピーされません。カードのある項目は先にローカルに設定できます。",
  "placementDefaults.confirm": "既定を確認",
  "placementDefaults.confirmAsk": "次回実行時、これがコピーされます:",
  "placementDefaults.unmatched": "ここに項目のない、バックアップ内の名前。チェックしたものは除外されます:",
  "placementDefaults.unmatchedLine": "{name}、スナップショット: {n}",
  "placementDefaults.confirmed": "既定が確認されました。オフサイトのコピーは次回実行時に再開します。",
  ...
  "placement.older": "{target} の古いコピー: {n}件、最終確認 {date}",
  "placement.noCopyYet": "{place}: まだコピーなし",
  "placement.notListedYet": "{place}: まだ未登録",

  // Removal
  "offsiteRemoval.delete": "{target} で削除",
  "offsiteRemoval.ask": "{target} にある {name} のすべてのコピーを削除しますか? コピー数: {n}。",
  "offsiteRemoval.onlyThere": "これらのスナップショットは他のどこにも存在せず、削除後は完全に失われます:",
