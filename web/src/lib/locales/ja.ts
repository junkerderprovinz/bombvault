...
const ja: Partial<Translations> = {
  ...
  "excludes.advisoryImmichDb": "Immich は写真ごとのアルバム、顔、日付を PostgreSQL のデータベースに保存しています。そのデータベースは、このコンテナとは別のコンテナで動いているため、このバックアップには含まれていません。ここから復元しても、写真は戻りますが、アルバムも顔も日付も戻りません。そのデータベース専用のバックアップを用意するか、Immich 内蔵のデータベースのエクスポートを有効にして、このバックアップが対象とするフォルダーに出力されるようにしてください。",
  "excludes.advisoryNextcloudDb": "Nextcloud はアカウント、共有、タグをデータベースに保存しています。そのデータベースは通常、別のコンテナで動いているため、このバックアップには含まれていません。ファイルは戻りますが、共有は戻りません。そのデータベースも併せてバックアップしてください。",
