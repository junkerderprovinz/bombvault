...
const ja: Partial<Translations> = {
  ...
  "dbdump.introNotice": "BombVault はバックアップのたびに、認識した {count} 件のデータベースを独立した復元ポイントとしてダンプします。コンテナごとにそのカードで、または設定ですべてのコンテナについて無効にできます。",
  ...
  "dbdump.fixRepository": "バックアップリポジトリがダンプを受け取らなかったか、ダンプされたより少ないバイト数しか保存しませんでした。設定のストレージでリポジトリを確認してください。ダンプは次のバックアップで再試行されます。",
  ...
  "recovery.dumpOnlySkipped": "ダンプとしてだけ保存されているデータベース {count} 件は対象外です。専用の一覧から復元して取り込んでください。",
