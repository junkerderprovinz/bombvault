...
const ko: Partial<Translations> = {
  ...
  "repos.itemHint": "이 항목의 백업이 어디로 갈지입니다. 이 항목에 전용 저장소가 필요한 경우가 아니면 도메인 저장소로 두세요. 흔한 이유는 거의 바뀌지 않는 큰 항목이고 외부 사본만 필요한 경우입니다. 목록에는 설정에서 만든 것이 들어 있습니다. 오프사이트 복제는 원격 저장소나 이미 오프사이트로 표시한 저장소를 두 번 복사하지 않습니다.",
  ...
  "repos.offsite": "이미 오프사이트",
  "repos.offsiteHint": "다른 기기의 공유 폴더용입니다. 예를 들어 콜드 스토리지로 쓰는 NAS입니다. 켜면 오프사이트 복제가 원격 저장소처럼 이 저장소를 제외하므로 백업이 두 번 복사되지 않습니다. 이 기기의 디스크나 공유 폴더라면 꺼 두세요. 그쪽은 여전히 오프사이트 사본이 필요합니다.",
  "repos.offsiteRemote": "오프사이트",
  "repos.offsiteRemoteHint": "원격 저장소는 이미 오프사이트에 있으므로 오프사이트 복제가 다시 복사하지 않습니다.",
