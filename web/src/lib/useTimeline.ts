 ...
 *  every other place only through loadPlace. `reading` holds the places whose
 *  read is still out, so a caller can lock what would ask for them again. */
export function useTimeline(domain: TimelineDomain, key: string, open: boolean) {
  ...
  const markReading = useCallback((place: string, busy: boolean) => {
    setReading((s) => {
      const next = new Set(s);
      if (busy) next.add(place);
  ...
  const fetchPlace = useCallback(
    async (place: string, gen: number) => {
      ...
      try {
        ...
        setPlaces((ps) =>
          ps.map((p) =>
            p.place !== place ? p : (fresh ?? { ...p, state: "unreadable" as const, error: res.error ?? "" })
          )
        );
        setRows((rs) => mergePlaceRows(rs, place, fresh ? (res.rows ?? []) : []));
      } finally {
        markReading(place, false);
      }
    },
    [domain, key, markReading]
  );
  ...
  return { places, rows, loading, error, reading, loadPlace, reload };
}
