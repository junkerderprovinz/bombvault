...
describe("taking over a renamed VM's entry", () => {
  ...
  it("is not offered on a card with former names and no last backup", () => {
    renderRow({ ...win11, aliases: ["1_windows-11"] }, ["1_debian-old"]);
  ...
  it("leaves out an entry whose name is another entry's former name", async () => {
    vi.mocked(listVMs).mockResolvedValue({
      ...
      vms: [
        win11,
        { ...win11, name: "debian", libvirtName: "1_debian", lastBackup: 1_757_000_000, aliases: ["1_debian-old"] },
        { ...win11, name: "1_debian-old", libvirtName: "1_debian-old", state: "not-installed" },
        { ...win11, name: "1_windows-11", libvirtName: "1_windows-11", state: "not-installed" },
      ],
    });
    render(
